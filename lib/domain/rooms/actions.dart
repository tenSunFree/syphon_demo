...
import 'package:syphon_demo/domain/events/actions.dart';
...
import 'package:syphon_demo/domain/user/actions.dart';
import 'package:syphon_demo/domain/user/model.dart';
import 'package:syphon_demo/global/libraries/matrix/errors.dart';
...
import 'package:syphon_demo/global/libraries/matrix/user/types.dart';
...
ThunkAction<AppState> fetchRooms({bool syncState = false}) {
  debugPrint('fetchRooms');
  return (Store<AppState> store) async {
    try {
      final data = await MatrixApi.fetchPhotos(
          // protocol: store.state.authStore.protocol,
          // homeserver: store.state.authStore.user.homeserver,
          // accessToken: store.state.authStore.user.accessToken,
          );
      debugPrint('fetchRooms, data: ${data.length}');
      var gg = data.map(
        (e) {
          // debugPrint('fetchRooms, url: ${e.url}');
          return Room(
            id: e.id.toString(),
            avatarUri: e.url,
            name: e.title,
      ...
      store.dispatch(SetRooms(rooms: gg));
